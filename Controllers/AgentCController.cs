...
{
    public class AgentCController : Controller
    {
        
        private readonly SSISContext _context;

        public AgentCController(SSISContext context)
        {
            _context = context;
        }
        //GET
        public IActionResult Dashboard()
        {
            ...
            {
                ViewData["Dept1"] = new SelectList(_context.Department, "Name", "Name");
                ViewData["Dept2"] = new SelectList(_context.Department, "Name", "Name");
                ViewData["Dept3"] = new SelectList(_context.Department, "Name", "Name");
                ViewData["Cate"] = new SelectList(_context.Category, "CatName", "CatName");
        ...
        //POST:generate trend report by department
        [HttpPost]
        public IActionResult Dashboard(string year1, string month1, string year2, string month2, string year3, string month3, string categories, string dept1, string dept2, string dept3)
        {
            
            ViewData["Dept1"] = new SelectList(_context.Department, "Name", "Name", dept1);
            ViewData["Dept2"] = new SelectList(_context.Department, "Name", "Name", dept2);
            ViewData["Dept3"] = new SelectList(_context.Department, "Name", "Name", dept3);
            ViewData["Cate"] = new SelectList(_context.Category, "CatName", "CatName", categories);
            List<string> yearl = new List<string> { (DateTime.Now.Year - 2).ToString(), (DateTime.Now.Year - 1).ToString(), (DateTime.Now.Year).ToString() };
            ...
            ViewBag.DateY1 = year1;
            ViewBag.Date2 = year2 + "-" + month2 + " ";
            ViewBag.Date3 = year3 + "-" + month3 + " ";
            //**line chart legend value
            ViewBag.lDept1 = dept1 + " ";
            ViewBag.lDept2 = dept2 + " ";
            ViewBag.lDept3 = dept3 + " ";
            //**line chart x-value
            ...
            var deptReqlist = from x in _context.DeptRequest.Include(p => p.Dept).Include(q => q.Item)
                              .Where(x => x.Item.Cat.CatName.Contains(n))
                              select x;
            ...
            foreach (DeptRequest dr in ilist)
            {
                for (int i = 0; i < 3; i++)
                {
                    if (dr.Dept.Name.Contains(xlist[i]))
                    {
